 ...
 * <code>probability</code> if its home cell is within the given x and y
 * coordinate range.
 * 
 * @author Sascha Holzhauer
 * 
 */
public class SpatialProbabilityRandomRecruitment implements
	...
	protected int xMax = Integer.MIN_VALUE;

	@Element(required = true)
	protected int xMin = Integer.MIN_VALUE;

	@Element(required = true)
	protected int yMax = Integer.MIN_VALUE;

	@Element(required = true)
	protected int yMin = Integer.MIN_VALUE;
	...
	public Collection<InnovationAgent> getRecruitedAgents(Collection<? extends Agent> allAgents) {
		...
		for (Agent agent : allAgents) {
			if (agent instanceof InnovationAgent
					&& agent.getHomeCell().getX() >= this.xMin
					&& agent.getHomeCell().getX() <= this.xMax

					&& agent.getHomeCell().getY() >= this.yMin
					&& agent.getHomeCell().getY() <= this.yMax
