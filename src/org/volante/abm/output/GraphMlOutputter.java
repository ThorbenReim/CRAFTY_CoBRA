...
public class GraphMlOutputter extends AbstractOutputter {
	...
	public void doOutput(Regions regions) {
		Map<String, GraphMLMetadata<Agent>> vertexMetadata = null;
		if (addCoordinates) {
			...
			vertexMetadata.put("Y", new GraphMLMetadata<Agent>("Y coordinate", "NA",
					new Transformer<Agent, String>() {
						@Override
						public String transform(Agent agent) {
		...
		if (addAFTSerialID) {
			...
			vertexMetadata.put("AFT", new GraphMLMetadata<Agent>("Agent Functional Type", "-1",
					new Transformer<Agent, String>() {
						@Override
						public String transform(Agent agent) {
