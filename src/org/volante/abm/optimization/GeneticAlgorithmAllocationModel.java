...
public class GeneticAlgorithmAllocationModel extends OptimizationAllocationModel<LandUseChromosome> {
	...
	/**
	 * What proportion of chromosomes are selected based on elitism (see
	 * http://commons.apache.org/math
	 * /apidocs/org/apache/commons/math3/genetics/ElitisticListPopulation.html)
	 */
	@Attribute(required = false)
	...
	LandUseChromosome doRun() {
		...
		StoppingCondition stopCond = new ConvergenceMaxStoppingCondition(numGenerations,
				convergenceProportion, convergenceGenerations);
	...
	public Population getInitialPopulation(Region r) {
		...
		for (int i = 0; i < numChromosomes; i++) {
			chromosomes.add(new LandUseChromosome(mutatedLandUseList(currentLandUse,
					initialMutationRate)));
		}
	...
	public List<PotentialAgent> mutatedLandUseList(List<PotentialAgent> initial, double rate) {
		...
		for (PotentialAgent p : initial) {
			if (region.getRandom().getURService()
					.getGenerator(RandomPa.RANDOM_SEED_RUN_ALLOCATION.name()).nextDouble() < rate) {
				agents.add(sample(available, region.getRandom().getURService(),
	...
	public String getOptimisationType() {
		return "Genetical Algorithms";
	}

	class LandUseChromosome extends AbstractListChromosome<PotentialAgent> {
		double	fitness	= -Double.MAX_VALUE;

		public LandUseChromosome(List<PotentialAgent> representation)
		...
		@Override
		protected void checkValidity(List<PotentialAgent> chromosomeRepresentation)
				throws InvalidRepresentationException {
		}

		@Override
		public AbstractListChromosome<PotentialAgent> newFixedLengthChromosome(
				List<PotentialAgent> chromosomeRepresentation) {
			return new LandUseChromosome(chromosomeRepresentation);
		}

		// Should be immutable list...
		List<PotentialAgent> getData() {
		...
		public Chromosome mutate(Chromosome original) throws MathIllegalArgumentException {
			if (!(original instanceof LandUseChromosome)) {
				throw new MathIllegalArgumentException(new DummyLocalizable(
						"Incompatible Chromosome" + original.getClass()));
			}
	...
	public class ConvergenceMaxStoppingCondition extends FixedGenerationCount {
		...
		public ConvergenceMaxStoppingCondition(int maxGenerations, double convergence, int gens)
