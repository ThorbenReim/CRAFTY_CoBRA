...
/**
 * Takes all the cells in the region, and assigns the agent which most optimises global fitness to
 * each cell.
 * 
 ...
 * 
 */
public class BestByCellGlobalAllocationModel extends
		OptimizationAllocationModel<List<PotentialAgent>> {
	...
	ConvergenceChecker	checker			= null;
	...
	List<PotentialAgent> doRun() {
		...
		while (!checker.isSatisfied()) {
			if (shuffle) {
				Utilities.shuffle(cells, region.getRandom().getURService(),
	...
	double assignBestAgent(Cell c, Collection<PotentialAgent> agents,
			List<PotentialAgent> currentSetup) {
