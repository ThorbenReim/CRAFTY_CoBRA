...
public abstract class OptimizationAllocationModel<T> implements AllocationModel {
 ...
 (SocialAgent) agent);
	...
	public double getCurrentFitness() {
		return calculateFitness(currentLandUseList());
	}
	...
	public double calculateFitness(List<PotentialAgent> agents) {
		...
		double fitness = region.getUnadjustedCompetitiveness(perCellResidual);
		return fitness;
	}
	...
	int cellToIndex(Cell c) {
		...
		// log.debug( String.format( "mx: %3d, my: %3d, cx: %3d, cy: %3d", e.getMinX(), e.getMinY(),
		// c.getX(), c.getY() ) );
		int regWidth = e.getWidth();
		int cxInd = c.getX() - e.getMinX();
		int cyInd = c.getY() - e.getMinY();
		int index = cxInd + (cyInd) * regWidth;
		// log.debug( String.format("cx: %3d, cy: %3d, rw: %3d, ind:%3d", cxInd, cyInd, regWidth,
		// index) );
		return index;
	}
	...
	public List<PotentialAgent> currentLandUseList() {
		...
		for (Cell c : region.getAllCells()) {
			...
			if (current == null) {
				log.error("GA Got null agent in cell " + c);
				throw new IllegalStateException("GA Got null agent in cell " + c);
			}
			if (agentNames.contains(current.getID())) {
				l.set(index, current);
			} else if (current.getID() != Agent.NOT_MANAGED_ID) {
	...
	public void addLogListener() {
		addListener(new LogOptimizationListener(Priority.DEBUG));
	}
	...
	public class LogOptimizationListener implements OptimizationListener {
		...
		public LogOptimizationListener(Priority level) {
			this.level = level;
		}

		@Override
		public void setState(String state) {
			log.log(level, "State: " + state);
		}

		@Override
		public void updateBest(int run, double score) {
			log.log(level, String.format("Run %4d. Best score: %6f", run, score));
		}

		@Override
		public void startRuns() {
			log.log(level, "Starting...");
		}
	}
}
