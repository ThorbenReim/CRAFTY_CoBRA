	...
	public static void testLowIntensityArableAgent(DefaultFR p) {
		SimpleProductionTest
				.testLowIntensityArableProduction((SimpleProductionModel) p
						.getProduction());
		assertEquals(0.5, p.getAssociatedGivingInThreshold(), 0.0001);
		assertEquals(1, p.getAssociatedGivingUpThreshold(), 0.0001);
	...
	public static void testHighIntensityArableAgent(DefaultFR p) {
		SimpleProductionTest
				.testHighIntensityArableProduction((SimpleProductionModel) p
						.getProduction());
		assertEquals(0.5, p.getAssociatedGivingUpThreshold(), 0.0001);
		assertEquals(1, p.getAssociatedGivingInThreshold(), 0.0001);
	...
	public static void testCommercialForestryAgent(DefaultFR p) {
		SimpleProductionTest
				.testCommercialForestryProduction((SimpleProductionModel) p
						.getProduction());
		assertEquals(0.1, p.getAssociatedGivingInThreshold(), 0.0001);
		assertEquals(3, p.getAssociatedGivingInThreshold(), 0.0001);
